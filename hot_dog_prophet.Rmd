...
```{r}
hd_sales <- read.csv(file = "data/hourly_combined_hd_sales.csv", header = TRUE) %>%
...
Now we need to create a set of training and test sets, some involving periods of holidays. Let's set the prediction periods to be 2 weeks long, this will be the timeframe of the test sets as well. Let's create 5 datasets for cross validation. All training periods will start wiht the first observation and ends with the start of the test period. This reflects how we would operate normally in production as well.

```{r}
model_ranges <- tibble(
  name = c("Christmas","Late winter","Easter","Early summer","Late summer"),
  start = as.Date(c("2022-12-19","2023-02-20","2023-04-03","2023-06-05","2023-08-14"))) %>%
  mutate(end = start + days(13)) %>%
...
So far this looks pretty promising, although it is quite hard to decide which model is better. Let's calculate some aggregate errors, like mean absolute error (MAE) and root mean square error (RMSE).
...
model_ranges_avg_calced %>%
  mutate(mae.n4 = map2(target, model_avg_N_4, 
                          function(x,y)left_join(x, y %>% rename(NUMBER_OF_PRODUCT_PRED = NUMBER_OF_PRODUCT_SOLD), by = "SES_TRX_DATEHOUR") %>%
                            mutate(ae = abs(NUMBER_OF_PRODUCT_PRED - NUMBER_OF_PRODUCT_SOLD)) %>%
                            pull(ae) %>%
                            mean()) %>% unlist()) %>%
  ...
  mutate(rmse.n4 = map2(target, model_avg_N_4, 
                          function(x,y)left_join(x, y %>% rename(NUMBER_OF_PRODUCT_PRED = NUMBER_OF_PRODUCT_SOLD), by = "SES_TRX_DATEHOUR") %>%
                            mutate(se = (NUMBER_OF_PRODUCT_PRED - NUMBER_OF_PRODUCT_SOLD)^2) %>%
                            pull(se) %>%
                            {sqrt(mean(.))}) %>% unlist()) %>%
...
The longer average model (N = 12) seems to outperform the shorter one (N = 4). Not necessarily what I have expected, but the difference is not huge.
...
plot(m, fcst) + xlim(max(future$ds) - weeks(4), max(future$ds))
```
...
```{r}
m2 <- prophet(
      growth = "linear", #linear or logistic or flat
      changepoints = NULL,
      n.changepoints = 25,
  ...
  add_country_holidays('HU') %>%
  fit.prophet(train %>% mutate(cap = 80) %>% mutate(floor = 0)) #cap and floor added for if logistic growth is selected
fcst2 <- predict(m, future %>% mutate(cap = 80) %>% mutate(floor = 0))
```

How does it look?

```{r}
plot(m2, fcst2) + xlim(max(future$ds) - weeks(4), max(future$ds))
```
